...
class PPOAgent:
    ...
    def __init__(self, env):
        ...
        self.Save_Path = 'ppo_models'
        self.state_size = self.env.state_space.shape #(self.REM_STEP, self.ROWS, self.COLS)

        if not os.path.exists(self.Save_Path): os.makedirs(self.Save_Path)
        self.path = '{}_APPO_{}'.format(self.env.name, self.lr)
        ...
        # for multiagent env... put these here unique to each agent
        self.score = 0
        self.pulls = np.zeros(self.env._arms,)
        self.arm_totals = np.zeros(self.env._arms,)
        self._state = self.env._state
    ...
    def act(self, state):
        ...
        action = np.random.choice(self.action_size, p=prediction) #sample with probabilities instead of just argmax
    ...
    def replay(self):
        ...
        self.Actor.fit(states, y_true, epochs=self.EPOCHS, verbose=0, shuffle=True, batch_size=len(self.rewards))
        self.Critic.fit(states, discounted_r, epochs=self.EPOCHS, verbose=0, shuffle=True, batch_size=len(self.rewards))
    ...
    def PlotModel(self, episode):
        self.scores.append(self.score)
        self.episodes.append(episode)
        self.average.append(sum(self.scores[-50:]) / len(self.scores[-50:]))
        # print("sum: ", self.scores[-1], " len: ", len(self.scores[-50:]))
    ...
    def train(self, n_threads):
        ...
        envs = [gym.make(self.env.name) for i in range(n_threads)]
    ...
    def train_threading(self, agent, env, thread):
        while self.episode < self.EPISODES:
            ...
            with self.lock:
                average = self.PlotModel(self.episode)
                # saving best models
                if average >= self.max_average:
                    self.max_average = average
                    self.save()
